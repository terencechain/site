...
[The last post](/writing/epbs-changes-how-ethereum-scales/) ended on an unresolved question. [ePBS](https://eips.ethereum.org/EIPS/eip-7732) separates the consensus block from the execution payload, and the consensus block is attested before the payload is revealed. Validators can agree that the consensus block arrived on time while disagreeing on whether its payload did. This post covers how ePBS resolves payload ambiguity: a sampled committee votes inside the slot on whether the payload showed up.

One naive solution: let the next slot's attesters vote on whether the previous slot's payload was revealed on time. This introduces the least amount of changes, since local fork choice rules are typically out of consensus. We could define a rule that declares a payload canonical, checked by the next slot's attesters: the payload arrived on time.

What this doesn't provide is a signal to the next slot's proposer. Attesters in the current slot vote 3 seconds into the slot, but the payload is revealed 6 seconds into the slot. The current slot's attestations carry no payload information. No aggregated view of the payload exists until the next slot's attestations, which arrive after the next slot's proposer has already committed to a parent.

This makes the next slot's proposer's job harder. It must build on either the block with its payload or the block without it, using only its local view of payload arrival. If it picks the side the majority did not see, its own block gets orphaned. A malicious builder can grief a single proposer node easier than a committee of nodes.

In Gloas, ePBS fixes this with a new voting round after the payload reveal deadline. Each slot, 512 validators are sampled from that slot's committees, weighted by balance. They are the Payload Timeliness Committee. At 9 seconds into the slot, each member broadcasts a signed vote signaling two things: did the payload arrive on time, and did the blobs. The vote is only for seeing the payload and blobs, not for executing them, so votes are fast to cast.

The deadline matters because a rational builder may want to reveal as late as possible. Until the payload is revealed, the builder is the only party that knows the post execution state, and that monopoly is valuable: competitors get less time to build the next block on top of it, and the builder can sell preconfirmations against a state only it can see. The reveal deadline bounds how much of the slot the winning builder's advantage can cover. The gap between the payload reveal and payload attestation intervals also ensures there's sufficient time for a builder or proposer to import and build a block for the next slot.
...
Payload timeliness is used in the fork choice rule. Every block in a slot has two possible states: full, the block with its payload, or empty, the block without it. The PTC quorum, a majority of more than 256 of the 512, is what the fork choice rule uses to choose between full and empty, and it only applies to the previous slot's block during the current slot. In later slots, regular attestation support takes over and the PTC votes no longer matter.

An honest proposer follows the PTC quorum: build on full if the committee saw the payload in time, otherwise build on empty. Its built block carries the full or empty choice along with the PTC votes, and the next slot's attesters vote to agree or disagree with it through their regular attestations.

The PTC quorum is a useful hint, not a validity rule. A proposer that imported the payload can still build on full even if the majority of the PTC voted not seen. Such is allowed, as long as the payload is valid and anyone can import it. It is a bet because attesters that imported the payload themselves can follow the block, attesters that never received it cannot vote for it, so the proposer is betting its own block on how much of the network actually has the payload.
...
The payload's state is decoupled from the block's state. If the payload is late or missing, the slot is not skipped. Contrary to today, where an invalid payload could kill the whole slot, in ePBS it only kills the payload. A PTC vote cannot make a missing payload appear: a node that never received the payload will not treat the block as full, no matter how the committee voted. The votes are signed and included on chain in the next block, leaving a record of what each member claimed to have seen.

This design protects both honest parties. The honest proposer picks a builder that later withholds, and its consensus block should still remain canonical. The slot becomes empty instead of being skipped. The honest builder reveals once the slot's attestation support is high enough and no proposer equivocation is visible. The support should beat proposer boost advantage, so the block cannot be orphaned by a proposer boosted block alone. Revealing under both conditions is proven safe: the payload becomes canonical, and the builder pays exactly its bid.
...
The last post covered how ePBS scales Ethereum by using more of the 12-second slot. That would only work if the network can agree on when the payload was revealed. Without a deadline enforced within the slot, you get ambiguity and gaming on when to reveal. A deadline measured by a sampled committee and published inside the slot resolves that ambiguity.
