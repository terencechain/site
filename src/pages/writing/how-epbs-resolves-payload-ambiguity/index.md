...
The last post [ePBS changes how Ethereum scales](/writing/epbs-changes-how-ethereum-scales/) ended on an unresolved question. [ePBS](https://eips.ethereum.org/EIPS/eip-7732) separates the consensus block from the execution payload, and the consensus block is attested before the payload is revealed. Validators can agree that the consensus block arrived on time while disagreeing on whether its payload did. This post covers how ePBS resolves payload ambiguity: a sampled committee votes inside the slot on whether the payload was revealed on time.
